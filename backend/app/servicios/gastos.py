...
def _aplicar_gasto_externo(sesion, id_cuenta, monto, descripcion,
                           quien_pago, id_grupo=None, fecha=None):
    """
    Aplica SIN commit un gasto que pagó alguien de afuera (ej. la cónyuge): el
    gasto se registra igual (cuenta en los reportes de gastos), pero NO reduce
    tus cuentas — lo financia un aporte externo del mismo monto (item 10b,
    replica el "gasto de otro lado que dio ingreso" del Excel).

    Concretamente: un INGRESO_EXTERNO que entra a la cuenta (+monto) y la
    SALIDA del gasto (-monto). El saldo de la cuenta queda IGUAL (neto cero);
    el gasto aparece como gasto y queda registrado quién lo cubrió.

    Sin commit para poder COMPONER varias líneas en una sola transacción (la
    tabla de gastos puede mezclar líneas propias y externas; ver gastos_lote.py),
    mismo patrón que _aplicar_gasto y _aplicar_compra.

    Devuelve: el Movimiento de SALIDA (el gasto en sí).
    """
    cuenta = sesion.get(Cuenta, id_cuenta)
    if cuenta is None:
        raise ValueError(f"No existe cuenta con Id {id_cuenta}")
...
def registrar_gasto_cubierto_externo(sesion, id_cuenta, monto, descripcion,
                                     quien_pago, id_grupo=None, fecha=None):
    """Registra un gasto cubierto por un aporte externo y hace commit.
    Envuelve a _aplicar_gasto_externo. Atómico."""
    try:
        gasto = _aplicar_gasto_externo(
            sesion, id_cuenta, monto, descripcion, quien_pago,
            id_grupo=id_grupo, fecha=fecha,
        )
        sesion.commit()
        return gasto
    except Exception as e:
